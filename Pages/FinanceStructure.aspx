<%@ Page Language="C#" AutoEventWireup="true" CodeFile="FinanceStructure.aspx.cs" Inherits="Pages_Finance_Structure" %>

<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
<title>Finance System</title>
<style>

*, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }

body {
    font-family: Arial, sans-serif;
    background: #eef0f2;
    color: #1a1a1a;
}

.sidebar {
    width: 80px;
    height: 100vh;
    background: black;
    position: fixed;
    top: 0;
    left: 0;
    display: flex;
    flex-direction: column;
    align-items: center;
    padding: 12px 0;
    z-index: 100;
}

.sidebar-logo {
    width: 44px;
    height: 44px;
    border-radius: 10px;
    overflow: hidden;
    display: flex;
    align-items: center;
    justify-content: center;
    margin-bottom: 4px;
    flex-shrink: 0;
    background: #000;
}

.sidebar-brand {
    font-size: 9px;
    font-weight: 700;
    color: rgba(255,255,255,0.85);
    letter-spacing: 0.12em;
    text-transform: uppercase;
    margin-bottom: 14px;
}
.sidebar-logo img {
    width: 100%;
    height: 100%;
    object-fit: cover;
}

.sidebar-nav {
    display: flex;
    flex-direction: column;
    align-items: center;
    gap: 4px;
    flex: 1;
    width: 100%;
}

.sidebar a {
    width: 80px;
    height: 40px;
    border-radius: 10px;
    display: flex;
    align-items: center;
    justify-content: center;
    color: rgba(255,255,255,0.7);
    text-decoration: none;
    position: relative;
    transition: background 0.15s, color 0.15s;
}
.sidebar a:hover {
    background: rgba(255,255,255,0.28);
    color: #fff;
}
.sidebar a.active {
    background: rgba(255,255,255,0.35);
    color: #fff;
}

.sidebar a::after {
    content: attr(data-tip);
    position: absolute;
    left: 52px;
    top: 50%;
    transform: translateY(-50%);
    background: #3a3a4a;
    color: #fff;
    font-size: 12px;
    padding: 5px 10px;
    border-radius: 6px;
    white-space: nowrap;
    opacity: 0;
    pointer-events: none;
    transition: opacity 0.15s;
    z-index: 200;
}
.sidebar a:hover::after { opacity: 1; }

.sidebar-sep {
    width: 28px;
    height: 1px;
    background: rgba(255,255,255,0.3);
    margin: 6px 0;
}

.topnav {
    position: fixed;
    top: 0;
    left: 80px;
    right: 0;
    height: 52px;
    background: #fff;
    border-bottom: 1px solid rgba(0,0,0,0.07);
    display: flex;
    align-items: center;
    padding: 0 28px;
    gap: 8px;
    z-index: 90;
}
.topnav-crumb { font-size: 12px; color: #9a9a9a; }
.topnav-arrow { font-size: 12px; color: #c0c0c0; }
.topnav-title { font-size: 14px; font-weight: 600; color: #1a1a1a; }

.main {
    margin-left: 80px;
    padding-top: 52px;
    min-height: 100vh;
    background: #eef0f2;
}

.page-body {
    padding: 28px 32px;
    display: grid;
    grid-template-columns: 360px 1fr;
    gap: 20px;
    align-items: start;
}

.form-container {
    background: #fff;
    border: 1px solid rgba(0,0,0,0.08);
    border-radius: 12px;
    overflow: hidden;
}

.form-header {
    padding: 16px 20px;
    border-bottom: 1px solid rgba(0,0,0,0.07);
    display: flex;
    align-items: center;
    gap: 10px;
}
.form-header-icon {
    width: 28px;
    height: 28px;
    border-radius: 7px;
    background: #f0f0f0;
    display: flex;
    align-items: center;
    justify-content: center;
}
.form-header h2 {
    font-size: 14px;
    font-weight: 600;
    color: #1a1a1a;
    margin: 0;
}
.form-header p {
    font-size: 11px;
    color: #9a9a9a;
    margin: 1px 0 0;
}

.form-body { padding: 20px; }

.search-wrap {
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    border: 1px solid rgba(0,0,0,0.12);
    border-radius: 8px;
    background: #f7f7f7;
    margin-bottom: 16px;
}
.search-wrap svg { flex-shrink: 0; }
.search-wrap .textbox {
    flex: 1;
    border: none !important;
    background: transparent !important;
    outline: none;
    padding: 0 !important;
    font-size: 13px;
    color: #1a1a1a;
    width: 100%;
}

.field-row {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 12px;
    margin-bottom: 12px;
}
.field-row .field { margin-bottom: 0; }

.field { margin-bottom: 12px; }

label {
    display: block;
    font-size: 11px;
    font-weight: 600;
    color: #9a9a9a;
    text-transform: uppercase;
    letter-spacing: 0.05em;
    margin-bottom: 5px;
}

.textbox {
    width: 100%;
    padding: 8px 10px;
    border: 1px solid rgba(0,0,0,0.12);
    border-radius: 8px;
    background: #fff;
    color: #1a1a1a;
    font-size: 13px;
    font-family: Arial, sans-serif;
    outline: none;
    transition: border-color 0.15s, box-shadow 0.15s;
}
.textbox:focus {
    border-color: #6b6b7e;
    box-shadow: 0 0 0 3px rgba(107,107,126,0.1);
}

.buttons {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px;
    margin-top: 18px;
}

.btn {
    padding: 9px 12px;
    border-radius: 8px;
    font-size: 13px;
    font-weight: 500;
    cursor: pointer;
    border: none;
    font-family: Arial, sans-serif;
    transition: opacity 0.15s;
}
.btn:hover { opacity: 0.85; }

#Button1 { background: #f0f0f0; color: #5f5f5f; border: 1px solid rgba(0,0,0,0.10); }
#Button2 { background: #4a4a5a; color: #fff; }
#Button3 { background: #3a7a5e; color: #fff; }
#Button4 { background: #f5eded; color: #8a3a3a; border: 1px solid #e0c0c0; }

.grid-card {
    background: #fff;
    border: 1px solid rgba(0,0,0,0.08);
    border-radius: 12px;
    overflow: hidden;
}

.grid-header {
    padding: 14px 20px;
    border-bottom: 1px solid rgba(0,0,0,0.07);
    display: flex;
    align-items: center;
    justify-content: space-between;
}
.grid-header-left {
    display: flex;
    align-items: center;
    gap: 10px;
}
.grid-header-icon {
    width: 28px;
    height: 28px;
    border-radius: 7px;
    background: #f0f0f0;
    display: flex;
    align-items: center;
    justify-content: center;
}
.grid-header h3 {
    font-size: 14px;
    font-weight: 600;
    color: #1a1a1a;
    margin: 0;
}

/* record count badge */
.record-count {
    font-size: 11px;
    color: #9a9a9a;
    background: #f0f0f0;
    padding: 3px 10px;
    border-radius: 20px;
}

.table {
    width: 100%;
    border-collapse: collapse;
    font-size: 13px;
}
.table th {
    font-size: 11px;
    font-weight: 600;
    color: #9a9a9a;
    text-align: left;
    padding: 10px 16px;
    background: #f7f7f7;
    border-bottom: 1px solid rgba(0,0,0,0.07);
    text-transform: uppercase;
    letter-spacing: 0.04em;
    white-space: nowrap;
}
.table td {
    padding: 11px 16px;
    border-bottom: 1px solid rgba(0,0,0,0.05);
    color: #1a1a1a;
    vertical-align: middle;
}
.table tr:last-child td { border-bottom: none; }
.table tr:hover td { background: #f7f7f7; cursor: pointer; }

.table tr.selected td,
.table .GridView_Selected td {
    background: #ececec !important;
}

.table input[type="submit"],
.table a {
    padding: 4px 12px;
    border-radius: 6px;
    border: 1px solid rgba(0,0,0,0.11);
    background: transparent;
    color: #5f5f5f;
    font-size: 12px;
    cursor: pointer;
    text-decoration: none;
    font-family: Arial, sans-serif;
    transition: background 0.15s;
}
.table input[type="submit"]:hover,
.table a:hover {
    background: #f0f0f0;
}

/* ── PAGINATION ── */
.pagination-bar {
    display: flex;
    align-items: center;
    justify-content: space-between;
    padding: 12px 16px;
    border-top: 1px solid rgba(0,0,0,0.06);
    background: #fafafa;
}

.pagination-info {
    font-size: 12px;
    color: #9a9a9a;
}

.pagination-controls {
    display: flex;
    align-items: center;
    gap: 4px;
}

/* GridView pager row cells become our buttons */
.table tr.pager td {
    padding: 0;
    border: none;
    background: transparent;
}

/* All pager links and spans */
.table tr.pager td table {
    border-collapse: separate;
    border-spacing: 3px;
}

.table tr.pager td a,
.table tr.pager td span {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    min-width: 30px;
    height: 30px;
    padding: 0 8px;
    border-radius: 7px;
    font-size: 12px;
    font-weight: 500;
    border: 1px solid rgba(0,0,0,0.10);
    background: #fff;
    color: #4a4a5a;
    cursor: pointer;
    text-decoration: none;
    transition: background 0.12s, border-color 0.12s;
}

.table tr.pager td a:hover {
    background: #f0f0f0;
    border-color: rgba(0,0,0,0.18);
}

/* Current page — filled dark */
.table tr.pager td span {
    background: #4a4a5a;
    color: #fff;
    border-color: #4a4a5a;
    cursor: default;
}

hr { display: none; }

@media (max-width: 900px) {
    .page-body { grid-template-columns: 1fr; }
}
</style>
</head>
    <invent-assistant assistant-id="ast_6m7o6yUd12iDwuRooEsDtW"></invent-assistant>
<script type="text/javascript" src="https://www.useinvent.com/embed.js" async defer></script>
<body>

<script>
    var currentPage = window.location.pathname.split("/").pop();
    var links = document.querySelectorAll(".sidebar a");
    links.forEach(function (link) {
        var href = link.getAttribute("href");
        if (href === currentPage) {
            link.classList.add("active");
        }
    });
</script>

<form id="form2" runat="server">
<asp:ScriptManager runat="server" />


<div class="sidebar">
    <div class="sidebar-logo">
        <img src="../Images/afl3.jfif.png" alt="AFMS Logo" />
    </div>
    <div class="sidebar-brand">AFMS</div>

    <div class="sidebar-nav">
        <a href="Dashboard.aspx" data-tip="Dashboard">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <rect x="2" y="2" width="5" height="5" rx="1" fill="currentColor" opacity="0.7"/>
                <rect x="9" y="2" width="5" height="5" rx="1" fill="currentColor" opacity="0.4"/>
                <rect x="2" y="9" width="5" height="5" rx="1" fill="currentColor" opacity="0.4"/>
                <rect x="9" y="9" width="5" height="5" rx="1" fill="currentColor" opacity="0.7"/>
            </svg>
        </a>
        <a href="FinanceStructure.aspx" data-tip="Finance Structure">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <rect x="2" y="3" width="12" height="2" rx="1" fill="currentColor"/>
                <rect x="2" y="7" width="8" height="2" rx="1" fill="currentColor" opacity="0.6"/>
                <rect x="2" y="11" width="5" height="2" rx="1" fill="currentColor" opacity="0.35"/>
            </svg>
        </a>
        <a href="JournalEntry.aspx" data-tip="Journal Entry">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path d="M3 5h10M3 8h7M3 11h5" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>
            </svg>
        </a>
        <a href="Ledger.aspx" data-tip="Ledger">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <rect x="3" y="2" width="10" height="12" rx="1.5" stroke="currentColor" stroke-width="1.3"/>
                <path d="M5 6h6M5 9h4" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/>
            </svg>
        </a>
        <a href="TrialBalance.aspx" data-tip="Trial Balance">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path d="M2 12l4-4 2 2 6-6" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
        </a>
        <a href="IncomeStatement.aspx" data-tip="Income Statement">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <rect x="2" y="2" width="12" height="12" rx="1.5" stroke="currentColor" stroke-width="1.3"/>
                <path d="M5 5h6M5 8h4M9 11l1.5-2 1.5 2" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
        </a>
        <a href="BalanceSheet.aspx" data-tip="Balance Sheet">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <rect x="2" y="2" width="12" height="12" rx="2" stroke="currentColor" stroke-width="1.3"/>
                <path d="M8 2v12M2 8h12" stroke="currentColor" stroke-width="1.2"/>
            </svg>
        </a>
    </div>

    <div class="sidebar-sep"></div>
    <a href="Login.aspx" data-tip="Logout">
        <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
            <path d="M5 13H3a1 1 0 01-1-1V3a1 1 0 011-1h2M10 10.5L13 7.5l-3-3M13 7.5H5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
    </a>
</div>

<div class="topnav">
    <span class="topnav-crumb">AFMS</span>
    <span class="topnav-arrow">›</span>
<span class="topnav-title">Finance Structure</span>

<div style="margin-left:auto; font-size:13px; font-weight:600; color:#4a4a5a;">
    Welcome,
    <asp:Label ID="lblUsername" runat="server"></asp:Label>
</div></div>

<div class="main">
<div class="page-body">

    <div class="form-container">
        <div class="form-header">
            <div class="form-header-icon">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                    <path d="M7 1v12M2 4h10M2 10h10" stroke="#6b6b7e" stroke-width="1.3" stroke-linecap="round"/>
                </svg>
            </div>
            <div>
                <h2>Account form</h2>
                <p>Fill details and save</p>
            </div>
        </div>

        <div class="form-body">

            <div class="search-wrap">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                    <circle cx="6" cy="6" r="4" stroke="#9a9a9a" stroke-width="1.3"/>
                    <path d="M10 10l2.5 2.5" stroke="#9a9a9a" stroke-width="1.3" stroke-linecap="round"/>
                </svg>
                <asp:TextBox ID="TextBox1" runat="server" CssClass="textbox"
                    AutoPostBack="true" OnTextChanged="TextBox1_TextChanged"
                    placeholder="Search account code..."></asp:TextBox>
            </div>

            <div class="field-row">
                <div class="field">
                    <label>Parent account code</label>
                    <asp:TextBox ID="TextBox2" runat="server" CssClass="textbox" placeholder="e.g. 1,2,3,4"></asp:TextBox>
                </div>
                <div class="field">
                    <label>Account code</label>
                    <asp:TextBox ID="TextBox3" runat="server" CssClass="textbox" placeholder="e.g. 1001"></asp:TextBox>
                </div>
            </div>

            <div class="field">
                <label>Account description</label>
                <asp:TextBox ID="TextBox4" runat="server" CssClass="textbox" placeholder="e.g. Cash and bank equivalents"></asp:TextBox>
            </div>

            <div class="field-row">
                <div class="field">
                    <label>Opening balance</label>
                    <asp:TextBox ID="TextBox5" runat="server" CssClass="textbox" placeholder="0.00"></asp:TextBox>
                </div>
                <div class="field">
                    <label>Balance status</label>
                    <asp:DropDownList ID="DropDownList1" runat="server" CssClass="textbox">
                        <asp:ListItem>Debit</asp:ListItem>
                        <asp:ListItem>Credit</asp:ListItem>
                    </asp:DropDownList>
                </div>
            </div>

            <div class="buttons">
                <asp:Button ID="Button1" runat="server" Text="Clear"  CssClass="btn" OnClick="Add_Click"/>
                <asp:Button ID="Button2" runat="server" Text="Save"   CssClass="btn" OnClick="Save_Click"/>
                <asp:Button ID="Button3" runat="server" Text="Update" CssClass="btn" OnClick="Update_Click"/>
                <asp:Button ID="Delete" runat="server" Text="Delete"  OnClick="Delete_Click" 
                    OnClientClick="return confirm('Are you sure you want to delete this account?');" />
            </div>

        </div>
    </div>

    <!-- GRID CARD -->
    <div class="grid-card">
        <div class="grid-header">
            <div class="grid-header-left">
                <div class="grid-header-icon">
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                        <rect x="1" y="2" width="12" height="10" rx="1.5" stroke="#6b6b7e" stroke-width="1.2"/>
                        <path d="M1 5.5h12M4.5 2v10" stroke="#6b6b7e" stroke-width="1.2"/>
                    </svg>
                </div>
                <h3>Chart of accounts</h3>
            </div>
            <!-- Live record count -->
            <span class="record-count">
                <asp:Label ID="lblRecordCount" runat="server" Text="0 records" />
            </span>
        </div>

        <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="false"
            CssClass="table" Width="100%"
            OnSelectedIndexChanged="GridView1_SelectedIndexChanged"
            OnPageIndexChanging="GridView1_PageIndexChanging"
            DataKeyNames="AccountCode"
            SelectedRowStyle-CssClass="GridView_Selected"
            AllowPaging="true"
            PageSize="10"
            PagerStyle-CssClass="pager">

            <PagerSettings
                Mode="NumericFirstLast"
                FirstPageText="«"
                LastPageText="»"
                PageButtonCount="5" />

            <Columns>
                <asp:BoundField DataField="ParentAccountCode"  HeaderText="Parent Code" />
                <asp:BoundField DataField="AccountCode"        HeaderText="Account Code" />
                <asp:BoundField DataField="AccountDescription" HeaderText="Description" />
                <asp:BoundField DataField="OpeningBalance"     HeaderText="Balance" />
                <asp:BoundField DataField="BalanceStatus"      HeaderText="Status" />
                <asp:CommandField ShowSelectButton="true" SelectText="Select" />
            </Columns>
        </asp:GridView>

    
        <div class="pagination-bar">
            <span class="pagination-info">
                <asp:Label ID="lblPageInfo" runat="server" Text="" />
            </span>
        </div>
    </div>

</div>
</div>

</form>
</body>
</html>
